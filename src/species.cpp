...
struct species *create_subject_1()
{
    ...
    subject->orient = rand();
    subject->red_color = 0;
    subject->green_color = 255;
    subject->blue_color = 255;
    subject->input_size = BASE_SIZE;
    subject->input_energy = UINT8_T_QUARTER_MAX;
    ...
    for (i=0; i<layer1_nb; i++)
    {
        ...
        int8_t **inputs = (int8_t**) malloc(layer1_nb*sizeof(int8_t*));
        inputs[0] = &(subject->input_energy);
        inputs[1] = &(subject->input_captor_red);
        inputs[2] = &(subject->input_captor_green);
        inputs[3] = &(subject->input_captor_blue);
        inputs[4] = &(subject->input_captor_feel);
        neurons[i].inputs = inputs;

        int8_t *weights = (int8_t*) malloc((layer1_nb+1)*sizeof(int8_t));
        weights[0] = rand()%4 - 1;
        weights[1] = rand()%4 - 1;
        weights[2] = rand()%4 - 1;
        weights[3] = rand()%4 - 1;
    ...
    for (i=0; i<layer2_nb; i++)
    {
        ...
        inputs[4] = &(neural->neuron_layers[0].neurons[4].output);
        neurons[i].inputs = inputs;
    ...
    neurons[0].weights[0] = -99; // j'évite d'attaquer tout le temps comme un mongole
    neurons[1].weights[0] = 99; //avancement constant
    /* fin section */
...
struct species *copy_species_1(struct species *subject)
{
    ...
    *new_subject = *subject;
    ...
    for (i=0; i<neuron_layers[0].layer_size; i++)
    {
        ...
        int8_t **inputs = (int8_t**) malloc(neuron_layers[0].layer_size*sizeof(int8_t*));
        ...
        int8_t *weights = (int8_t*) malloc((neuron_layers[0].layer_size+1)*sizeof(int8_t));
    ...
    for (i=0; i<neuron_layers[1].layer_size; i++)
    {
        ...
        int8_t **inputs = (int8_t**) malloc(neuron_layers[1].layer_size*sizeof(int8_t*));
        ...
        int8_t *weights = (int8_t*) malloc((neuron_layers[1].layer_size+1)*sizeof(int8_t));
...
void reproduce(struct species *subject)
{
    ...
    if (species != NULL) {
        ...
        new_species->subject->orient += UINT8_T_HALF_MAX;
...
{
    if (world_timestamp_return() % 32 == 0)
    ...
    if (subject->input_energy > 0) {
        ...
        for (k=0; k<subject->neural.layer_nb; k++)
        {
            for (j=0; j<subject->neural.neuron_layers[k].layer_size; j++)
            {
                ...
                int8_t **inputs = subject->neural.neuron_layers[k].neurons[j].inputs;
                int8_t *weights = subject->neural.neuron_layers[k].neurons[j].weights;
        ...
        if (subject->output_turn_left)
            turn_left(subject);
        if (subject->input_energy > UINT8_T_QUARTER_MAX)
            reproduce(subject);
    }  else {
        printf("someone died :'(\n");
        world_species_destroy(subject);
    }
}
...
void display_subject_1(struct species *subject, uint8_t display_type)
{
    ...
    if (subject->output_attack) {
        ...
        uint32_t x2 = subject->pos_x + 10*subject->input_size*cos((subject->orient)/TWOPI - PI_ON_EIGHT);
        uint32_t y2 = subject->pos_y + 10*subject->input_size*sin((subject->orient)/TWOPI - PI_ON_EIGHT);
        uint32_t x3 = subject->pos_x + 10*subject->input_size*cos((subject->orient)/TWOPI + PI_ON_EIGHT);
        uint32_t y3 = subject->pos_y + 10*subject->input_size*sin((subject->orient)/TWOPI + PI_ON_EIGHT);
...
void release_species(struct species *subject)
{
    ...
    free(subject);
    subject_number--;
    printf("nombre encore en vie : %d\n", subject_number);
    if (subject_number == 2 && graphic_is_open()) {
        (*world_species_return())->subject->input_energy = UINT8_T_HALF_MAX;
        (*world_species_return())->next->subject->input_energy=UINT8_T_HALF_MAX;
    }
}
